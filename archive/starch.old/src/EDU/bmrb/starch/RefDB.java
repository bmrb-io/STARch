 ...
 * All Rights Reserved.
 *
 * FILE:        $Source: /cvs_archive/cvs/starch/src/EDU/bmrb/starch/RefDB.java,v $
 * 
...
if( DEBUG ) System.err.println( "ID for " + name + " nomenclature = " + rc );
    ...
    throws java.sql.SQLException {
        ...
        sql.append( EDU.bmrb.nmrstar.utils.QuoteString.quoteForDB( residue ) );
...
// expand
    ...
    /** Returns true if atom is a "special-case" atom ("any" residue).
     * @param nomid nomenclature id
     * @param restype residue type id
     * @param atom atom name
     * @return true or false
     */
    public boolean isAnyAtom( int nomid, int restype, String atom )
    throws java.sql.SQLException {
        boolean rc = false;
        java.sql.Statement query = fConn.createStatement( java.sql.ResultSet.TYPE_FORWARD_ONLY,
        java.sql.ResultSet.CONCUR_READ_ONLY );
if( DEBUG ) System.err.println( "checking any atom " + atom );
        java.sql.ResultSet rs = query.executeQuery( "SELECT a.ID FROM ATOMS a,RESIDUES r " +
        " WHERE r.LABEL='X' AND r.TYPEID=" + restype + " AND a.PSEUDO='Y' AND a.NAME=" +
        EDU.bmrb.nmrstar.utils.QuoteString.quoteForDB( atom ) + " AND a.RESID=r.ID" );
        if( rs.next() ) rc = true;
        rs.close();
        query.close();
        return rc;
...
if( DEBUG ) System.err.println( "failed check restype " + residue );
        ...
        sql.append( EDU.bmrb.nmrstar.utils.QuoteString.quoteForDB( rc ) );
        sql.append( " AND a.PSEUDO='Y' AND r.LABEL='X' AND r.TYPEID=" );
        sql.append( typeid );
        sql.append( " AND a.RESID=r.ID" );
if( DEBUG ) System.err.println( sql );
        rs = query.executeQuery( sql.toString() );
