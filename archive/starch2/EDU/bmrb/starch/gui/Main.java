...
//FIXME: remove hardcode?
        ...
        for( int i = 0; i < fGrid.getColumnCount(); i++ ) {
            ...
            if( fGrid.getColumnName( i ).equals( "Author_seq_code" )
            || fGrid.getColumnName( i ).equals( "Author_comp_code" )
            || fGrid.getColumnName( i ).equals( "Author_atom_code" ) )
...
// limit to main button only
        if( evt.getButton() == java.awt.event.MouseEvent.BUTTON1 ) {
            ...
            try {
                ...
                if( end < 0 ) return;
                String atom = msg.substring( start + 1, end );
                for( int i = 0; i < fGrid.getRowCount(); i++ ) {
                    if( fGrid.getValueAt( i, 9 ).toString().equals( seq )
                    && fGrid.getValueAt( i, 11 ).toString().equals( atom ) ) {
